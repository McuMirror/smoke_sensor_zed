# <a id="Top">Smoke Sensor Zigbee Telink TLSR8258 (E-BYTE E180-Z5812SP)</a>
...
* Подключиться к сети zigbee - удерживать кнопку нажатой 2 секунды (подтверждение включение зеленого светодиода на 1.5 секунды).
* Покинуть сеть zigbee - удерживать кнопку нажатой 2 секунды (подтверждение включение зеленого светодиода на 1.5 секунды).
* Сделать restart модуля - нажать кнопку 5 раз (подтверждение включение зеленого светодиода на 1.5 секунды).
...
Первые 30 секунд модуль просыпается раз в 3 секунды. По истечению 30 секунд модуль засыпает на 5 минут. И такой период сна в 5 минут остается до тех пор, пока не произойдет одно из событий

1. Обнаруживается задымление.
2. Обнаруживается отсоединение от крепежной пластины.
3. Нажали кнопку на плате.

При первых двух событиях модуль просыпается, отсылает отчет и опять засыпает на 5 минут.
При третьем событии модуль начинает просыпаться раз в 3 секунды в течении 20 секунд. Далее период сна опять возвращается к 5 минутам.

**Reporting**

Модуль высылает 2 отчета для батарейки - процент заряда и напряжение. Периода отправки нет, отчет высылается по изменению. Изменение учитывает 0.1 вольт. Рабочим считается напряжение от 9.0 вольт до 7.2. Сам извещатель начиная с 7.2 вольта до 5.9 переходит в режим оповещения о разряде батареи - подает кратковременный однократный звуковой сигнал с периодом повторения от 60 до 65 секунд.
...
Далее, вставляем батарейки в устройство. Если питание было уже подано, то удерживаем кнопку на плате устройства (не на извещателе) более 2 секунд, до подтверждения сводиодом долгой вспышкой. Устройство должно подключиться к сети zigbee. Если подключение прошло удачно, то мы обнаружим наше устройство в zigbee2mqtt.
